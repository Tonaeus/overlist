...
const getList = async (req, res) => {
    ...
    try {
        ...
        const list = await List.findById({ _id: id, uid });
        if (!list) {
            res.status(404).json({ error: "No such list." });
            return;
        }
...
const getLists = async (req, res) => {
    try {
        const uid = req.user?.id;
        const lists = await List.find({ uid });
...
const createList = async (req, res) => {
    ...
    try {
        const uid = req.user?.id;
        const newList = await List.create({ label, uid });
        await ListHeader.create({ list_id: newList._id, uid });
        await ListBody.create({ list_id: newList._id, uid });
...
const updateList = async (req, res) => {
    ...
    try {
        const uid = req.user?.id;
        const list = await List.findOneAndUpdate({ _id: id, uid }, { label }, { new: true });
        if (!list) {
            res.status(404).json({ error: "No such list." });
            return;
        }
...
const updateLists = async (req, res) => {
    ...
    try {
        ...
        const updateResult = await List.updateMany({ _id: { $in: ids }, uid }, { $set: { directory_id } });
        if (updateResult.modifiedCount !== ids.length) {
            throw new Error();
        }
        const updatedLists = await List.find({ _id: { $in: ids }, uid });
...
const deleteLists = async (req, res) => {
    ...
    try {
        ...
        const deleteResult = await List.deleteMany({ _id: { $in: ids }, uid });
        const headerResult = await ListHeader.deleteMany({ list_id: { $in: ids }, uid });
        const bodyResult = await ListBody.deleteMany({ list_id: { $in: ids }, uid });
...
const copyLists = async (req, res) => {
    ...
    try {
        ...
        const listsToCopy = [];
        const listHeadersToCopy = [];
        const listBodiesToCopy = [];
        ids.forEach(id => {
...
export { getList, getLists, createList, updateList, updateLists, deleteLists, copyLists, };

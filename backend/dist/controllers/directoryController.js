...
const updateDirectory = async (req, res) => {
    ...
    try {
        ...
        const directory = await Directory.findOneAndUpdate({ _id: id, uid }, { label }, { new: true });
        if (!directory) {
            res.status(404).json({ error: "No such directory." });
            return;
        }
...
const deleteDirectory = async (req, res) => {
    ...
    try {
        ...
        const updateResult = await List.updateMany({ directory_id: id, uid }, { $set: { directory_id: null } });
...
export { getDirectories, createDirectory, updateDirectory, deleteDirectory, };
